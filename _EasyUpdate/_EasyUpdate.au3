...
Func _EasyUpdate($iniURL, $sciptName, $curVersion)
	...
	$iscriptName = IniRead($iniFilePath, "EasyUpdate", "scriptName", 0)
	$urlDownload = IniRead($iniFilePath, "EasyUpdate", "scriptUrl", 0)
	$lastedVersion = IniRead($iniFilePath, "EasyUpdate", "scriptVersion", 0) ;
	$dateUpdate = IniRead($iniFilePath, "EasyUpdate", "dateUpdate", 0) ;
	$logContent = StringReplace(IniRead($iniFilePath, "EasyUpdate", "changeLog", 0), "<br>", @CRLF)
